...
const Movie = (props) => {
  ...
  const handleSearch = (event) => {
    ...
    let dataFilter = dataMovie.filter(value => {
            ...
            <div tw="py-10">
              <div tw="flex mb-8">
                <div tw="w-full xl:w-1/2">
                  <Input placeholder="search movie..." onChange={debouncedChangeHandler} />
                </div>
              </div>
              <>
                {(itemMovie.length > 0) ? (
                  <div tw="grid grid-cols-2 gap-3 md:grid-cols-3 md:gap-4 xl:grid-cols-5 xl:gap-5">
                    {itemMovie.map((item, key) => {
                      return (
...
export default connect(mapProps, mapDispatch)(Movie);
